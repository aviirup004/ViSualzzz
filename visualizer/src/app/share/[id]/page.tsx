...
import { Terminal, Save, Layers, MemoryStick, Cpu, AlertCircle, List, ArrowUpRight, Activity, Server, Zap, Timer, Loader2, ArrowLeft } from "lucide-react";
...
export default function SharedSnapshotPage() {
    ...
    const { setCodeSnippet, callStack, webApis, microtaskQueue, macrotaskQueue, nextTickQueue, checkQueue, consoleOutput } = useEngineStore();
    ...
    if (loading) {
        return (
            <div className="bg-[#0F172A] font-sans text-slate-100 h-screen flex flex-col items-center justify-center">
                <Loader2 size={48} className="animate-spin text-[#0dccf2] mb-4" />
                <h2 className="text-xl font-bold tracking-widest uppercase">Loading Snapshot...</h2>
            </div>
        );
    }
    ...
    if (notFound) {
        return (
            <div className="bg-[#0F172A] font-sans text-slate-100 h-screen flex flex-col items-center justify-center">
                ...
                <p className="text-slate-400 mb-8 max-w-md text-center">The snapshot you're looking for doesn't exist or has been removed.</p>
                <Link href="/dashboard" className="bg-[#0dccf2] text-[#0F172A] px-6 py-3 rounded-lg font-bold flex items-center gap-2 hover:brightness-110 transition-all">
    ...
    return (
        <div className="bg-[#0F172A] font-sans text-slate-100 overflow-hidden h-screen flex flex-col selection:bg-[#0dccf2]/30">
            ...
            <header className="flex items-center justify-between border-b border-slate-800 px-6 py-3 bg-[#0F172A]/80 backdrop-blur-md z-50">
                <Link href="/dashboard" className="text-3xl md:text-4xl font-black tracking-tight flex select-none cursor-pointer font-display text-white drop-shadow-lg">
                ...
                <div className="flex items-center gap-4">
                    <span className="bg-stone-600/80 text-xs px-2.5 py-1 rounded-full font-semibold border border-stone-500/50 shadow-sm cursor-default">View Only Snapshot</span>
                    {/* Environment Toggle */}
                    ...
                    <div className="flex items-center gap-2 bg-slate-800/50 rounded-lg px-3 py-1.5 border border-slate-700">
                        ...
                        <span className="text-xs font-mono text-slate-300">v8: Shared View</span>
                    </div>
                </div>
            </header>
